...
if ($engineer_id != '') {
    $pgfl = "SELECT `status`,`status_by_te`,`device_id` FROM $engineer_master WHERE `eid`='$engineer_id'";
    ...
    if ($totalresfl > 0) {
        ...
        if ($the_status == "INACTIVE") {
            ...
            if ($device_id_en != $en_device_id) {
                ...
                $force_logout_message = "Another device has logged in. Please try again.";

                // Handle force logout for the previous device
                handleForceLogout('engineer', $engineer_id, $conn);
            }
...
if ($te_code != '') {
    ...
    if ($totalresfl > 0) {
        ...
        if ($device_id_te != $te_device_id) {
            ...
            // Handle force logout for the previous device
            handleForceLogout('technician', $te_code, $conn);
        }
    } else {
        $te_force_logout = "YES";
        $force_logout_message = "Your details not found. Please contact admin.";

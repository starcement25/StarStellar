        ...
        if (mysqli_num_rows($res2) > 0) {
            ...
            if ($row2["gift_type_id"] == $giftTypeID && ($e_email == "" || !filter_var($e_email, FILTER_VALIDATE_EMAIL))) {
                ...
                $TDS = get_value_by_setting_key($conn, "TDS");
                    ...
                    // echo"<pre>";print_r($total_point); echo "<br>";
                    // print_r($tds_point);echo "<br>";
                    // print_r($TDS);echo "<br>";
                    // print_r($e_points);echo "<br>";  

                    // print_r($gift_point);
                    // die;
                    //add condition to check if engineer has sufficient points to redeem the gift sk 27-03-26
                    $total_point = is_numeric($total_point) ? (float)$total_point : 0;
                    $e_points    = is_numeric($e_points) ? (float)$e_points : 0;
                if ($total_point > $e_points) {
                    ...
                    if ($resmkord) {
                        ...
                        $sqlldgrin = "INSERT INTO $ledger_master 
                                      (`user_id`,`gift_order_master_id`, `description`, `point_redeem`, `ldgr_type`, `related_id`, `ldgr_datetime`, `remaining_balance`) 
                                      VALUES 
                                      ('$the_engineer_id','$the_order_id', '$gift_title', '$total_point', 'GIFT_REDEEM', '$gift_id', '$datetime', '$rest_points_for_engineer')";

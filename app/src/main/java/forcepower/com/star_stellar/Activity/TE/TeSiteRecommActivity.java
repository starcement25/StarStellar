...
import forcepower.com.star_stellar.Activity.TE.Adapter.ApproveAdapter;
...
import forcepower.com.star_stellar.Activity.TE.Adapter.PendingAdapter;
import forcepower.com.star_stellar.Activity.TE.Adapter.OnLoadMoreListener_P;
import forcepower.com.star_stellar.Activity.TE.Adapter.RejectAdapter;
...
import static forcepower.com.star_stellar.Class.AllUrl.ws_show_approved_recommended_site_for_te;
import static forcepower.com.star_stellar.Class.AllUrl.ws_show_pending_recommended_site_for_te;
import static forcepower.com.star_stellar.Class.AllUrl.ws_show_rejected_recommended_site_for_te;
...
public class TeSiteRecommActivity extends BaseActivity {
    Activity myActivity;
    ...
    private ApproveAdapter aAdapter;
    private RejectAdapter rAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            myActivity = TeSiteRecommActivity.this;
            //Header_View
            ...
            tvCaption.setText("List of Site Recommendations");
            ...
            pAdapter = new PendingAdapter(myActivity, p_list, rv_Pending);
            rv_Pending.setAdapter(pAdapter);

            handler_a = new Handler();
            aAdapter = new ApproveAdapter(myActivity, approve_list, rv_Approved);
            rv_Approved.setAdapter(aAdapter);

            handler_r = new Handler();
            rAdapter = new RejectAdapter(myActivity, reject_list, rv_Rejected);
    ...
    public void get_Pending_List(int page_no, String the_max_date_time, final String type) {
        ...
        client.post(ws_show_pending_recommended_site_for_te, params, new AsyncHttpResponseHandler() {
            @Override
                        ...
                        if (ja.length() > 0) {
                            for (int i = 0; i < ja.length(); i++) {
                                ...
                                p_list.add(new Student(e.getString("r_site_name"),
                                        e.getString("r_submission_date_modified"),
                                        e.getString("r_status"),
                                        e.toString(),
                                        e.getString("r_recomended_by")));
    ...
    public void get_Approve_List(int page_no, String the_max_date_time, final String type) {
        ...
        final AsyncHttpClient client = new AsyncHttpClient();
        client.setTimeout(DEFAULT_TIMEOUT);
        client.post(ws_show_approved_recommended_site_for_te, params, new AsyncHttpResponseHandler() {
            @Override
            public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
                final String str = new String(responseBody);
                try {
    ...
    public void get_Reject_List(int page_no, String the_max_date_time, final String type) {
        ...
        final AsyncHttpClient client = new AsyncHttpClient();
        client.setTimeout(DEFAULT_TIMEOUT);
        client.post(ws_show_rejected_recommended_site_for_te, params, new AsyncHttpResponseHandler() {
            @Override
            public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
                final String str = new String(responseBody);
                try {
                    ...
                    if (reader.optString("process_status").equalsIgnoreCase("YES")) {
                        ...
                        if (ja.length() > 0) {
                            for (int i = 0; i < ja.length(); i++) {
                                ...
                                if (i == 0 && the_max_date_time_R.matches("")) {

...
public class TeRecommendationDetails_A extends BaseActivity
{
    Activity myActivity;
    String json_row = "";
    ListView lv_recom_details, lv_te_eng, lv_te_status;
    private ArrayList<CommonHelper> menu_item_list = new ArrayList<>();
    private ArrayList<CommonHelper> eng_details_list = new ArrayList<>();
    private ArrayList<CommonHelper> status_list = new ArrayList<>();
    MySiteDetailsAdapter myAdapter, engAdapter, statusAdapter;
    @Override
    protected void onCreate(Bundle savedInstanceState)
    {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.te_recommendation_approved);

        try
        {
            myActivity = TeRecommendationDetails_A.this;
            //Header_View
            ...
            lv_te_status = (ListView) findViewById(R.id.lv_te_status);
            ...
            statusAdapter = new MySiteDetailsAdapter(this, status_list);
            lv_recom_details.setAdapter(myAdapter);
            lv_te_eng.setAdapter(engAdapter);
            lv_te_status.setAdapter(statusAdapter);
    ...
    private void parse_eng_details(String str)
    {
        try
        {
            final JSONObject e = new JSONObject(str);
    ...
    private void parse_status(String str)
    {
        try
        {
            final JSONObject e = new JSONObject(str);
            ...
            if(e.has("approved_date"))
            {
                cdh = new CommonHelper();
                cdh.setItem0("Approved Date");
                cdh.setItem1(e.getString("approved_date"));
                status_list.add(cdh);
            }

            if(e.has("stellar_points_earned"))
            {
                cdh = new CommonHelper();
                cdh.setItem0("Stellar Points Earned");
                cdh.setItem1(e.getString("stellar_points_earned"));
                status_list.add(cdh);
            }
            if(e.has("comments"))
            {
                cdh = new CommonHelper();
                cdh.setItem0("Comments");
                cdh.setItem1(e.getString("comments"));
                status_list.add(cdh);
            }

            statusAdapter.setFilter(status_list);
            setListViewHeightBasedOnItems(lv_te_status);
        }
        catch (final Exception e)
        {
            e.printStackTrace();

...
if(@$_POST["edit_slider_btn"]=="Update"){
	...
	$slider_visible_order = $_POST["slider_visible_order"];
...
$sqlsldupd = "update $home_screen_slider_for_engineer set `slider_header_text`='$add_sld_title',`slider_description_text`='$add_sld_des',`slider_image`='$ad_new_name',`slider_category`='$add_sl_slider_category' `slider_visible_order`=$slider_visible_order where `id`='$slidr_hid_id'";
$ressldupd = mysqli_query($conn,$sqlsldupd);
				$msg_txt = "Slider successfully updated.1";
				header("location:".$page_name."?msg_txt=".$msg_txt);
			}else{
$sqlsldupd = "update $home_screen_slider_for_engineer set `slider_header_text`='$add_sld_title',`slider_description_text`='$add_sld_des',`slider_category`='$add_sl_slider_category' `slider_visible_order`=$slider_visible_order where `id`='$slidr_hid_id'";
$ressldupd = mysqli_query($conn,$sqlsldupd);
...
header("location:".$page_name."?msg_txt=".$msg_txt);
		...
		echo $sqlsldupd;
		$ressldupd = mysqli_query($conn,$sqlsldupd);
		$msg_txt = "Slider successfully updated..".$sqlsldupd;
		header("location:".$page_name."?msg_txt=".$msg_txt);
		}		
}	
}
...
if(@$_POST["add_slider_btn"]=="Save"){
	...
	if($add_sld_title==""){
		...
		$msg_txt = "Please select a image.";
			...
			$upload_the_ad_file = move_uploaded_file($add_sldr_img_file_tmp_name,$target_file);
			if($upload_the_ad_file){
				$sqlsldin = "insert into $home_screen_slider_for_engineer (`slider_header_text`,`slider_description_text`,`slider_image`,`slider_category`,`slider_visible_order`) values('$add_sld_title','$add_sld_des','$ad_new_name','$add_sl_slider_category','$slider_visible_order')";
...
if($totressldck>0){
	...
	$slider_visible_order = $rowsldck["slider_visible_order"];
...
<section class="content">
    ...
    <label class="blbl">Slider order</label>
    <input type="number" class="form-control" id="slider_visible_order" name="slider_visible_order" placeholder="Enter Slider order" value="<?php echo $slider_visible_order;?>"> 
...
if($totres1>0){
	while($row1=mysqli_fetch_assoc($res1)){
		...
		$slider_visible_order = $row1["slider_visible_order"];
...
?>
<tr>
	<td><?php echo $slider_visible_order;?></td>
<td>
<?php
...
?>
<a href="<?php echo $page_name;?>?sedtid=<?php echo $the_sid;?>" class="btn bg-red waves-effect" style="margin-right:20px;">Edit</a>
<a href="javascript:void(0);" the_dlt_sid="<?php echo $the_sid;?>" class="btn bg-red waves-effect sdlt_btn">Delete</a>
<?php	
}
?>
</td>

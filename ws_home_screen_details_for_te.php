...
if($te_code!=''){
$te_data = array();
	$sql2 = "select `te_name`,`te_id`,`te_code` from $te_master where `te_code`='$te_code'";
...
$sql_slider = "select `slider_header_text`,`slider_description_text`,`slider_image`,`slider_category` from $home_screen_slider_for_engineer order by `id` asc";
...
$res_data = array("process_status"=>"YES","process_message"=>"Success.","te_name"=>$te_name,"number_of_points"=>$number_of_points,"top_section_header_text"=>$top_section_header_text,"top_section_description_text"=>$top_section_description_text,"top_section_image_link"=>$top_section_image_link,"offer_slider_data"=>$offer_slider_data,"featured_slider_data"=>$featured_slider_data);
	}else{
		$res_data = array("process_status"=>"NO","process_message"=>"The TE details doesn't exist.");
	}
}else{	

...
//$te_mobile_no = $row3["te_mobile_no"] ? trim($row3["te_mobile_no"]) : "";
			...
			$confirm_recomendation_point = $product_data_arr["point_per_bag"];
				...
				if ($confirm_more_than_bags > 0 && $actual_consumption >= $confirm_more_than_bags) {
					...
					$actual_recomendation_point_total = $actual_recomendation_point+$each_verified_site;
				}
...
// echo $e_mobile;
		...
		// sendSMSNotification($te_mobile_no, $congratulatory_message_te);
						...
						$l_points=$e_points+$actual_recomendation_point;
						$sqlldgrin = "insert into $ledger_master (user_id,description,point_earned,ldgr_type,related_id,ldgr_datetime,remaining_balance) values ('$r_engineer_id','Site Recomendation','$actual_recomendation_point','SITE_RECOMENDATION','$r_site_id','$r_submission_date','$l_points')";
						$resldgrin = mysqli_query($conn, $sqlldgrin);

						if ($confirm_more_than_bags > 0 && $actual_consumption >= $confirm_more_than_bags) {

							// Separate entry for bonus points in the ledger
							$l_points=$l_points+$confirm_bonus_points;

							$sqlldgrin_bonus = "insert into $ledger_master (user_id,description,point_earned,ldgr_type,related_id,ldgr_datetime,remaining_balance) values ('$r_engineer_id','Bonus Points','$confirm_bonus_points','BONUS','$r_site_id','$r_submission_date','$l_points')";
						...
						if ($existing_id != "") {
							...
							$l_points=$l_points+$each_verified_site;

							$sqlldgrin = "insert into $ledger_master (user_id,description,point_earned,ldgr_type,related_id,ldgr_datetime,remaining_balance) values ('$r_engineer_id','Verified Recomendation Site','$each_verified_site','SITE_RECOMENDATION','$r_site_id','$r_submission_date','$l_points')";
							$resldgrin = mysqli_query($conn, $sqlldgrin);

...
$the_eid = $_REQUEST["the_engineer_id"] ? addslashes(trim($_REQUEST["the_engineer_id"])) : "";
$last_update_datetime = $_REQUEST["last_update_datetime"] ? addslashes(trim($_REQUEST["last_update_datetime"])) : "";
if($the_eid!=""){
$sql2 = "select `te_code` from $engineer_master where `eid`='$the_eid'";
$res2 = mysqli_query($conn,$sql2);
$row2 = mysqli_fetch_assoc($res2);
$the_te_code = $row2["te_code"] ? trim($row2["te_code"]) : "";
if($the_te_code!=""){
...
$sqlall = "select * from $notification_message where (IF(`user_type` = 'SINGLE_ENGINEER', IF(`single_user_id` = '$the_eid', 1, 0), 0) = 1) or (IF(`user_type` = 'BRANCH_WISE_ENGINEER', IF(FIND_IN_SET(`branch_code`, '".$the_branch_code_str."'), 2, 0), 0) = 2) or (`user_type`='ALL' or `user_type`='ENGINEER' or `user_type`='BRANCH_WISE_ENGINEER')  order by `date_time` desc limit $start_from,$limit";

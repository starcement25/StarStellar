...
$sql2cnt = "select $recommended_site_master.`r_site_id` from $recommended_site_master left join $te_master on $recommended_site_master.`r_te_code`=$te_master.`te_code` where $recommended_site_master.`r_engineer_id`='$the_engineer_id' and $recommended_site_master.`r_status`='$the_status' $max_date_qry";
$res2cnt = mysqli_query($conn,$sql2cnt);
$tot_res2cnt = mysqli_num_rows($res2cnt);
$tot_res2cnt = intval($tot_res2cnt);

$sql2 = "select *,$te_master.`te_name` from $recommended_site_master left join $te_master on $recommended_site_master.`r_te_code`=$te_master.`te_code` where $recommended_site_master.`r_engineer_id`='$the_engineer_id' and $recommended_site_master.`r_status`='$the_status' $max_date_qry order by $recommended_site_master.`r_submission_date` desc limit $start_from,$limit";
...
$res_data = array("process_status"=>"NO","process_message"=>"No record found.","rejected_recommendation_data"=>$rejected_recommendation_data);
}
}else{

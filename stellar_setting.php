...
$sql_prod = "select * from $product_master order by `prod_id` asc";
...
$sql_prod2 = "select * from $product_master order by `prod_id` asc";
$row_prod_bonus = mysqli_query($conn, $sql_prod2);
$totres_prod_bonus = mysqli_num_rows($res_prod2);
...
$app_setting_key_arr = array("signup_point"=>"","site_approved_point"=>"","birthday_point"=>"","anniversary_point"=>"","each_verified_site"=>"","bags_verification_limit_for_te"=>"");
...
<tr>
    <td>
        ...
        <select id="sel_prod_id_bonus" class="sel_prod_id_bonus" autocomplete="off">
            <option value="">Select product</option>
            <?php
            if ($totres_prod_bonus > 0 && isset($res_prod_bonus)) {

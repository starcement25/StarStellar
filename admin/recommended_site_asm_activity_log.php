...
$msg_txt = "";
$add_page_name = "recommended_site_asm_activity_log.php";
$page_name = "recommended_site_asm_activity_log.php";
...
foreach($search_array as $search_array_key=>$search_array_val){
	if($search_array_key=="sl_ord_sts"){
		if($search_array_val!=''){
			...
			$whr_str .= "$aand $recommended_site_asm_activity_log.`r_status`='$search_array_val' ";
...
$whr_str .= "$aand ($te_master.`zone` like '%D%' or $te_master.`zone` like '%E%' ) ";
		...
		if($search_array_val!=''){
			...
			$whr_str .= "$aand $recommended_site_asm_activity_log.`r_te_code`='$search_array_val' ";	
		...
		if($search_array_val!=''){
			...
			$whr_str .= "$aand $recommended_site_asm_activity_log.`r_engineer_id`='$search_array_val' ";	
		...
		if($search_array_val!=''){
			...
			$whr_str .= "$aand ($engineer_master.`e_name` like '%$search_array_val%' or $engineer_master.`te_code` like '%$search_array_val%' or $engineer_master.`e_mobile` like '%$search_array_val%' or $te_master.`te_name` like '%$search_array_val%' or $te_master.`te_mobile_no` like '%$search_array_val%' or $recommended_site_asm_activity_log.`r_site_name` like '%$search_array_val%' or $recommended_site_asm_activity_log.`r_contact_person_name` like '%$search_array_val%' or $recommended_site_asm_activity_log.`r_mobile_no` like '%$search_array_val%' or $recommended_site_asm_activity_log.`r_address` like '%$search_array_val%' or $recommended_site_asm_activity_log.`r_site_potential_in_mt` like '%$search_array_val%' or $recommended_site_asm_activity_log.`r_contact_person_category_name` like '%$search_array_val%' )";
		...
		if($the_sl_day_wise=="Date_Range"){
			...
			if($the_from_dt!="" && $the_to_dt!=""){
			   $whr_str .= "$aand $recommended_site_asm_activity_log.`r_submission_date` between '".$the_from_dt." ".$frm_hrs."' and '".$the_to_dt." ".$to_hrs."' ";
			   $new_qry_string_filtered .= "&from_dt=".$the_from_dt."&to_dt=".$the_to_dt;
				...
				$whr_str .= "$aand $recommended_site_asm_activity_log.`r_submission_date` >= '".$the_from_dt." ".$frm_hrs."' ";
				...
				$whr_str .= "$aand $recommended_site_asm_activity_log.`r_submission_date` <= '".$the_to_dt." ".$to_hrs."' ";
			...
			if($the_sl_day_wise=="Today"){
				...
				$whr_str .= "$aand $recommended_site_asm_activity_log.`r_submission_date` between '".$current_date." ".$frm_hrs."' and '".$current_date." ".$to_hrs."' ";
				...
				$whr_str .= "$aand $recommended_site_asm_activity_log.`r_submission_date` between '".$yesterday_date." ".$frm_hrs."' and '".$yesterday_date." ".$to_hrs."' ";
...
$pgsql = "select $recommended_site_asm_activity_log.`rsaal_id` from $recommended_site_asm_activity_log left join $engineer_master on $recommended_site_asm_activity_log.`r_engineer_id`=$engineer_master.`eid` left join $te_master on $recommended_site_asm_activity_log.`r_te_code`=$te_master.`te_code` $new_whr_str ";
        ...
        <div class="container-fluid">
            ...
            <div class="row clearfix">
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="card">
                        <div class="header">
<h2>ASM Activity Log (<?php echo $total_pgres;?>)&nbsp;&nbsp;&nbsp;<span class="rpt_loader"></span> <span class="shomsg"><?php if($msg_txt!=""){ echo $msg_txt;}?></span>
...
<div class="wrapper_scrl">
 ...
 <div class="table-responsive tr_for_scroll">
                                <table class="table table-bordered table-striped table-hover table_for_scroll">
                                    ...
                                    <tbody>
<?php
$sql1 = "select $recommended_site_asm_activity_log.*,$engineer_master.`e_name`,$engineer_master.`e_mobile`,$engineer_master.`branch_code`,$te_master.`te_name`,$te_master.`te_mobile_no` from $recommended_site_asm_activity_log left join $engineer_master on $recommended_site_asm_activity_log.`r_engineer_id`=$engineer_master.`eid` left join $te_master on $recommended_site_asm_activity_log.`r_te_code`=$te_master.`te_code` $new_whr_str order by CAST($recommended_site_asm_activity_log.`r_site_id` as UNSIGNED) desc limit $start_from,$limit";
$res1 = mysqli_query($conn,$sql1);
$totres1 = mysqli_num_rows($res1);
if($totres1>0){
	while($row1=mysqli_fetch_assoc($res1)){
...
<td>
<div>
<?php echo '<span class="recomSiteEachField"><b>ID:</b> '.$the_r_site_id.'<strong id="rtoa_ldr_'.$the_r_site_id.'" class="rtoa_ldr"></strong></span>';?>
<?php if($r_status!=""){
if($r_status=="REJECTED"){

echo '<span class="recomSiteEachField"><b>Status:</b> <strong class="'.$stsClass.'"><a href="javascript:void(0);" class="rejectToPendingBtn">'.$r_status.'</a></strong></span>';
}else{
...
<?php if($r_te_interaction_date!=""){echo '<span class="recomSiteEachField"><b>Interacted On:</b></span><span class="recomSiteEachField">'.$r_te_interaction_date.'</span>';}?>
<?php if($r_te_interaction_comment!=""){echo '<span class="recomSiteEachField"><b>Comment:</b></span><span class="recomSiteEachField">'.$r_te_interaction_comment.'</span>';}?>

<?php if($r_asm_name!=""){echo '<span class="recomSiteEachField"><b>ASM Name:</b></span><span class="recomSiteEachField">'.$r_asm_name.'</span>';}?>
<?php if($r_asm_ph_no!=""){echo '<span class="recomSiteEachField"><b>Ph No.:</b></span><span class="recomSiteEachField">'.$r_asm_ph_no.'</span>';}?>
<?php if($r_asm_email!=""){echo '<span class="recomSiteEachField"><b>Email:</b></span><span class="recomSiteEachField">'.$r_asm_email.'</span>';}?>
<?php if($r_asm_branch!=""){echo '<span class="recomSiteEachField"><b>Branch:</b></span><span class="recomSiteEachField">'.$r_asm_branch.'</span>';}?>
...
<?php if($the_branch_code_selected!=""){echo '<span class="engineerEachField"><b>Branch:</b></span><span class="engineerEachField">'.$the_branch_code_selected.'</span>';}?>
</div>
</td>
<td>
<div>

<?php
  ...
  error_reporting(E_ALL);
    ...
    session_start();
    //echo $_SERVER['REQUEST_METHOD'].'###'.$_POST['csrf_token'].'@@'.$_SESSION['csrf_token'];die;
    ...
    if(($handle = fopen($destination, "r")) !== false)
    {
        ...
        while(($data = fgetcsv($handle)) !== false)
        {
            ...
            $category_name = isset($data[5]) ? trim($data[5]) : null;
            ...
            //category name
            $category_id = null;
            $giftcate = "select * from `categories` where upper(name)='".strtoupper($category_name)."' limit 1";
            $giftcatSql = mysqli_query($conn,$giftcate);
            while ($row1 = mysqli_fetch_assoc($giftcatSql)) {
                $category_id = $row1['id'];
            }
            
            if(empty($category_id))
            {
                $unprocessedMessages[] = 'Gift Category Name is empty or wrong in row '.$rowCount.'.<br>';
                $unprocessed++;
                continue;
            }
        ...
        //========================================
            ...
            $stmt->bind_param("siisssss", $gift_title, $gift_type_id, $category_id, $description, $image_name, $gift_point, $cur_time, $admin_id);

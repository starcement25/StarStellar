...
ini_set('display_startup_errors', 1);
	...
	$giftTypeQuery = "select * from $gift_types";
	$giftTypes = mysqli_query($conn,$giftTypeQuery);
	$totalGiftTypes = mysqli_num_rows($giftTypes);
	...
	function isGiftTypeValid($giftTypeID)
	{
		global $gift_types;
		global $conn;

		$giftTypeQueryByID = "select * from $gift_types where id = $giftTypeID";
	...
	function isCategoryValid($categoryId)
	{
		...
		$categoryQueryByID = "select * from categories where id = $categoryId and status = 1";
		...
		function insertGiftLog($gift_id, $action_type)
		{
			global $conn, $gift_master;
			global $cur_time;
			...
			$sql_old = "SELECT * FROM $gift_master WHERE id = '".intval($gift_id)."'";
			...
			$log_data = array(
				...
				'gift_title'    => $old['gift_title'],
				'category_id'   => $old['category_id'],
				'description'   => $old['description'],
				'gift_image'    => $old['gift_image'],
				'point_require' => $old['point_require'],
				'gift_type_id'  => $old['gift_type_id']
			);
			...
			/* Insert log */
			$log_sql = "
				INSERT INTO gift_master_log
				(gift_id, action_type, action_by, action_at, log_data)
				VALUES (
					'".intval($old['id'])."',
					'".$action_type."',
					'".intval($admin_id)."',
					'".$cur_time."',
					'".mysqli_real_escape_string($conn, $log_json)."'
				)
			";
	...
	$new_qry_string_filtered = "";
	$srch_gift_dtls = $_GET["srch_gift_dtls"] ? addslashes(trim($_GET["srch_gift_dtls"])) : "";
	$whr_str = "";
	$msg_txt = "";
	$search_array = array("srch_gift_dtls"=>$srch_gift_dtls);
	foreach($search_array as $search_array_key=>$search_array_val)
	{
		if($search_array_key=="srch_gift_dtls")
		{
			if($search_array_val!='')
			{
				//if(trim($whr_str)!=""){
				...
				$whr_str .= "$aand ($gift_master.`gift_title` like '%$search_array_val%' or $gift_master.`description` like '%$search_array_val%') ";
				$new_qry_string_filtered .= "&srch_gift_dtls=".$search_array_val;
			}
		}
	}
	...
	if(isset($_GET["dlt_sid"]) && @$_GET["dlt_sid"]!="")
	{
		...
		insertGiftLog($dlt_sid, 'DELETE');
		if($dlt_sid!="")
		{		
			$sqldel = "update $gift_master set is_deleted='yes'  where `id`='$dlt_sid'";
			$resdel = mysqli_query($conn,$sqldel);
		}
		$msg_txt = "Gift details successfully deleted.";
	...
	if(isset($_GET["the_dlt_gid"]) && @$_GET["the_dlt_gid"]!="")
	{
		...
		if($the_dlt_gid!="")
		{
			...
			insertGiftLog($the_dlt_gid, 'DELETE');
	...
	if(@$_POST["edit_gft_btn"]=="Update")
	{
		...
		$add_gift_type = $_POST["gift_type_id"] ? addslashes(trim($_POST["gift_type_id"])) : "";
		$add_category = $_POST["category_id"] ? addslashes(trim($_POST["category_id"])) : "";
		...
		else if($add_gift_type == "")
		{
			$msg_txt = "Please enter gift type.";
		}
		else if(!isGiftTypeValid($add_gift_type))
		{
			$msg_txt = "Invalid gift type.";
		}
		else if($add_category == "")
		{
			$msg_txt = "Please enter category.";
		}
		else if(!isCategoryValid($add_category))
		{
			$msg_txt = "Please enter a valid category.";
		}
		else
		{
			if($add_gft_img_file_name!="")
			{
				...
				else
				{
					...
					if($upload_the_ad_file)
					{
						...
						insertGiftLog($gft_hid_id, 'UPDATE');
						$sqlsldupd = "update $gift_master set `gift_title`='$add_gft_title', `category_id`='$add_category', `description`='$add_gft_des',`gift_type_id`='$add_gift_type',`point_require`='$add_gft_point',`gift_image`='$ad_new_name', last_updated_at='$cur_time',
    					last_updated_by='$admin_id' where `id`='$gft_hid_id'";
					...
					else
					{
						...
						$sqlsldupd = "update $gift_master set `gift_title`='$add_gft_title', `category_id`='$add_category', `description`='$add_gft_des',`gift_type_id`='$add_gift_type',`point_require`='$add_gft_point', last_updated_at='$cur_time',
    					last_updated_by='$admin_id' where `id`='$gft_hid_id'";
						$ressldupd = mysqli_query($conn,$sqlsldupd);
	...
	if(@$_POST["add_gft_btn"]=="Save")
	{
		...
		else
		{
			...
			else
			{
				...
				if($upload_the_ad_file)
				{
					$sqlsldin = "insert into $gift_master (`gift_title`, `category_id`, `description`,`gift_image`,`point_require`, `gift_type_id`,`last_updated_at`,`last_updated_by`) values('$add_gft_title', '$add_category', '$add_gft_des','$ad_new_name','$add_gft_point','$add_gift_type','$cur_time','$admin_id')";
	...
	if(isset($_GET["gedtid"]) && @$_GET["gedtid"]!="")
	{
		...
		if($totressldck>0)
		{
			...
			$selected_gift_type_id = $rowsldck["gift_type_id"] ? trim($rowsldck["gift_type_id"]) : "";
			$selected_category_id = $rowsldck["category_id"] ? trim($rowsldck["category_id"]) : "";
	...
	$pgsql = "select `id` from $gift_master WHERE is_deleted='no' $new_whr_str";
        ...
        <div class="row clearfix">
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                <div class="card">
                    <div class="header">
                        <h2>
							Gift Master (<?php echo $total_pgres;?>)&nbsp;&nbsp; <a href="export_gift_master.php" class="btn bg-red waves-effe">Export&nbsp;Gift&nbsp;Master</a> &nbsp;&nbsp; <a href="add_gift_master.php" class="btn bg-red waves-effe">Upload&nbsp;Gift&nbsp;Master</a> &nbsp; &nbsp;<a href="update_gift_master.php" class="btn bg-red waves-effe">Edit Bulk Upload&nbsp;Gift&nbsp;Master</a> &nbsp; &nbsp;<span class="shomsg"><?php if($msg_txt!=""){ echo $msg_txt;}?></span>
                        </h2>
                    	<div class="row clearfix">
							<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12 add_top_bottom_padding">
								<input type="text" class="form-control" id="srch_gift_dtls" value="<?php echo $srch_gift_dtls;?>" placeholder="Search Gift Details">
							</div>
							...
							<div class="col-lg-2 col-md-2 col-sm-12 col-xs-12 add_top_bottom_padding"></div>
                        </div>
                        ...
                        <div class="row clearfix">
							<form name="hss_form" id="hss_form" enctype="multipart/form-data" method="POST">
								...
								<div class="col-lg-3 col-md-4 col-sm-12 col-xs-12 add_top_bottom_padding">
								...
								<div class="col-lg-2 col-md-1 col-sm-12 col-xs-12 add_top_bottom_padding" style="text-align:left !important;">
									<label class="blbl">Gift Type</label>
									<select class="form-control sl_eng_sts" name="gift_type_id" id="id_gift_type_id" >
										<?php
											if($totalGiftTypes > 0)
											{
												while($giftType = mysqli_fetch_assoc($giftTypes))
												{ 
										?>
													<option value="<?php echo $giftType["id"]; ?>" <?php if($selected_gift_type_id == $giftType["id"]){?> selected="selected" <?php } ?>><?php echo $giftType["name"]; ?></option>
										<?php 
												}
											}
										?>
								...
								<div class="col-lg-2 col-md-1 col-sm-12 col-xs-12 add_top_bottom_padding" style="text-align:left !important;">
									<label class="blbl">Category</label>
									<select class="form-control sl_eng_sts" name="category_id" id="id_category_id" >
										...
										<?php
											if($totalCategories > 0)
											{
												...
												{ 
										?>
													<option value="<?php echo $category["id"]; ?>" <?php if($selected_category_id == $category["id"]){?> selected="selected" <?php } ?>><?php echo $category["name"]; ?></option>
										<?php 
												}
											}
										?>
						...
						<div class="wrapper_scrl">
 						...
 						<div class="table-responsive tr_for_scroll">
                            <table class="table table-bordered table-striped table-hover table_for_scroll">
                                ...
                                <tbody>
									<?php //commend by sk 02-02-26
										/*$sql1 = "select $gift_master.*, $gift_types.name AS gift_type_name from $gift_master JOIN $gift_types ON $gift_types.id = $gift_master.gift_type_id WHERE $gift_master.is_deleted='no' $new_whr_str order by ABS(`point_require`) asc limit $start_from,$limit";*/
										// $sql1 = "select $gift_master.*, $gift_types.name AS gift_type_name from $gift_master JOIN $gift_types ON $gift_types.id = $gift_master.gift_type_id WHERE $gift_master.is_deleted='no' $new_whr_str order by $gift_master.status,ABS(`point_require`) asc limit $start_from,$limit";

										$sql1 = "select $gift_master.*, $gift_types.name AS gift_type_name, categories.name AS category_name from $gift_master JOIN $gift_types ON $gift_types.id = $gift_master.gift_type_id 
										LEFT JOIN categories ON categories.id = $gift_master.category_id
										WHERE $gift_master.is_deleted='no' $new_whr_str order by $gift_master.status, ABS(`point_require`) asc limit $start_from,$limit";
										...
										if($totres1>0)
										{
											while($row1=mysqli_fetch_assoc($res1))
											{
												...
												$the_category_name = $row1["category_name"];
												$the_gift_description = $row1["description"];
												$the_gift_type_name = $row1["gift_type_name"];
									...
									?>
												<tr>
													...
													<td><?php echo $the_category_name;?></td>
													<td><?php echo $the_gift_description;?></td>
													<td><?php echo $the_gift_type_name;?></td>
														...
														<div>
															...
															?>
																	...
																	<span class="adminActivityField2">
																		<?php 
																			if(!in_array($_SESSION["menu_id"],$ediit_inactive_menu_array))
																			{
																		?>
																				<a href="<?php echo $add_page_name;?>?gedtid=<?php echo $the_gid;?>" class="btn bg-red waves-effe" style="margin-right:10px;">Edit</a>
																		<?php 
																			}
																		?>
																	</span>
													...
													<td>
														<div>
															<span class="adminActivityField2">
																...
																<?php 
																	if(strtoupper($the_access_user_type)=="ADMIN")
																	{
																?>
																		<a href="javascript:void(0);" class="btn bg-red waves-effe delete_gift" dlt_sid="<?php echo $the_gid;?>">Delete</a>
																<?php 
																	}
																?> 
															</span>
													...
													<td>
														<div>
															<span class="adminActivityField">
																<select class="form-control sl_redeem_sts" id="sl_redeem_sts<?php echo $the_gid;?>" gft_id="<?php echo $the_gid;?>">
																	<?php
																		if(count($em_status_arr)>0)
																		{
																			foreach($em_status_arr as $em_status_arr_val)
																			{ 
																	?>
																				<option value="<?php echo $em_status_arr_val;?>" <?php if($em_status_arr_val==$the_gift_status1){?> selected="selected" <?php } ?>><?php echo $em_status_arr_val;?></option>
																	<?php 
																			}
																		}
																	?>
															...
															<span class="adminActivityField3" id="sl_redeem_sts_ldr_<?php echo $the_gid;?>"></span>
														</div>
													</td>
												</tr>
...
<script type="text/javascript">
	jQuery(function(){
		...
		jQuery('.sl_redeem_sts').change(function(){
			var redeem_sts = jQuery(this).val();
			var gft_id = jQuery(this).attr("gft_id");
		
			if (redeem_sts != '' && gft_id != '') 
			{
				var ldr_elmnt = jQuery("#sl_redeem_sts_ldr_" + gft_id);
				ldr_elmnt.html(imgs);
			
				jQuery.ajax({
					url: 'ajax_update_gift_redeem_status.php',
					type: 'post',
					dataType: 'json',
					data: "gft_id=" + gft_id + "&redeem_sts=" + redeem_sts,
					success: function(response)
					{                
						if (response.process_sts == "YES") 
						{                    
		...
		jQuery(".srch_btn").click(function(){
			...
			if(srch_gift_dtls!="")
			{
				if(qstring!="")
				{
					qstring = qstring+"&srch_gift_dtls="+encodeURIComponent(srch_gift_dtls);
				}
				else
				{
					qstring = qstring+"srch_gift_dtls="+encodeURIComponent(srch_gift_dtls);
		...
		jQuery(".delete_gift").click(function(){
			var dlt_sid = jQuery(this).attr("dlt_sid");
			if(dlt_sid!="")
			{
				var cb = confirm("Do you want to delete this gift details?");
				if(cb == true)
				{

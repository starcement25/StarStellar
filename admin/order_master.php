...
$start_stellar_admin_name = $_SESSION["start_stellar_admin_name"];
...
$order_pending_reason = "order_pending_reason";

// getting the pedning remarks
$reasfltr = "select `reason` from $order_pending_reason order by `id` asc";
$reasonfltr = mysqli_query($conn,$reasfltr);
//$order_status_arr = array("PENDING","DELIVERED","REJECT");
//if($start_stellar_admin_name=='admin' || $start_stellar_admin_name=='accounts'){
...
$whr_str .= "$aand ($te_master.`zone` like '%D%' or $te_master.`zone` like '%E%' ) ";
		...
		if($search_array_val!=''){
			...
			$whr_str .= "$aand ($engineer_master.`e_name` like '%$search_array_val%' or $engineer_master.`te_code` like '%$search_array_val%' or $engineer_master.`e_mobile` like '%$search_array_val%' or $te_master.`te_name` like '%$search_array_val%' or $te_master.`te_mobile_no` like '%$search_array_val%' or $gift_master.`gift_title` like '%$search_array_val%' or $gift_order_master.`g_order_id` like '%$search_array_val%' or `order_id` like '%$search_array_val%' or $gift_order_master.`address` like '%$search_array_val%' or $gift_order_master.`city` like '%$search_array_val%' or $gift_order_master.`pin` like '%$search_array_val%' or $gift_order_master.`state` like '%$search_array_val%' )";
        ...
        <div class="container-fluid">
            ...
            <div class="row clearfix">
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="card">
                        <div class="header">
<h2>Order Master (<?php echo $total_pgres;?>)&nbsp;&nbsp;&nbsp;<a href="export_order_master.php?paged=<?php echo $page.$export_filtered_str;?>" class="btn bg-red waves-effe">Export</a> &nbsp;&nbsp;<span class="rpt_loader"></span> <span class="shomsg"><?php if($msg_txt!=""){ echo $msg_txt;}?></span>&nbsp;&nbsp;&nbsp;<a href="add_order_master.php" class="btn bg-red waves-effe">Bulk Upload Order Status</a>
...
<select class="form-control admin_sl_ord_sts" id="admin_sl_ord_sts_<?php echo $the_g_order_id;?>" data-order-id="<?php echo $the_g_order_id;?>">
...
if (count($order_status_arr) > 0) {
	foreach ($order_status_arr as $order_status_arr_val2) {
		...
		if($start_stellar_admin_name=='admin' || $start_stellar_admin_name=='accounts'){
			...
			if ($order_status_arr_val2 == 'REJECT'){
				$isDisabled = 'disabled';
			}
		}
		?>
...
<span class="adminActivityField" <?php if($r_status !="PENDING"){?> style="display:none" <?php }?>>
	<select class="form-control admin_sl_ord_sts_reason" id="admin_sl_ord_sts_reason_<?php echo $the_g_order_id;?>">
		<option>Choose Pending Reason </option>
		<?php
		
			while($row1reasonfltr=mysqli_fetch_assoc($reasonfltr)){
				$pend_reason = $row1reasonfltr["reason"];
				
		?>
		<option value="<?php echo $pend_reason;?>" <?php if($pend_reason == $pending_reason){?> selected="selected" <?php } ?> ><?php echo $pend_reason;?></option>
		<?php
			}
...
jQuery(".admin_update_order_data").click(function(){
	...
	if(the_ord_id!=""){
		...
		var pending_reason = jQuery("#admin_sl_ord_sts_reason_"+the_ord_id).val();
		...
		jQuery.ajax({
				...
				type: 'post',
				dataType: 'json',
				data: "the_ord_id="+the_ord_id+"&admin_sl_ord_sts="+admin_sl_ord_sts+"&admin_del_dt="+admin_del_dt+"&admin_amz_ord_id="+admin_amz_ord_id+"&admin_old_status="+admin_old_status+"&admin_amz_ord_link="+admin_amz_ord_link+"&admin_remarks="+admin_remarks+"&pending_reason="+pending_reason,
...
jQuery(".admin_sl_ord_sts").change(function(e){
		var sl_orderID = jQuery(this).data('order-id');
		var current_order_status = jQuery('#admin_sl_ord_sts_'+sl_orderID).val();
		console.log(sl_orderID);
		if(current_order_status=='PENDING'){
			jQuery('#admin_sl_ord_sts_reason_'+sl_orderID).show();
		}else{
			jQuery('#admin_sl_ord_sts_reason_'+sl_orderID).hide();
		}

...
$output .= '"TE CODE","ZONE","TE NAME","BRANCH","LINKED ENGINEER","SITES RECOMMENDED","SITES APPROVED","SITES PENDING","SITES REJECTED","GIFT REDEEMED","GIFT DELIVERED","QUERY RAISED","QUERY SOLVED","ACTIVE LINKED ENGINEER","SEMIACTIVE LINKED ENGINEER","INACTIVE LINKED ENGINEER"';

$output .="\n";


$sql1 = "select * from $te_master $new_whr_str order by `zone` asc";
$res1 = mysqli_query($conn,$sql1);
$totres1 = mysqli_num_rows($res1);
if($totres1>0){
	while($row1=mysqli_fetch_assoc($res1)){
...
$output .= '"'.$the_te_code.'","'.$the_te_zone.'","'.$the_te_name.'","'.$the_branch_name_selected_short.'","'.$total_linked_engineer.'","'.$total_site_recomended.'","'.$total_site_approved.'","'.$total_site_pending.'","'.$total_site_rejected.'","'.$gift_redemed.'","'.$gift_delevered.'","'.$query_raised.'","'.$query_solved.'","'.$total_active_engineer.'","'.$total_semi_active_engineer.'","'.$total_inactive_engineer.'"';
$output .="\n";

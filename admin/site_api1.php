        ...
        if ($row) {
            if (strpos($row['branch_code'], ',') !== false) {
                ...
                foreach ($BranchList as $Brvalue) {
                    $branch_list_data[] = array("br_cod" => $Brvalue);
                }
            } else {
                $branch_list_data[] = array("br_cod" => $row['branch_code']);
            }

            if (strpos($row['zone'], ',') !== false) {
                $ZonList = explode(',', $row['zone']);  // Change implode to explode
                foreach ($ZonList as $Znvalue) {
                    $zone_list_data[] = array("zn_cod" => $Znvalue); // Change "br_cod" to "zn_cod"
                }
            } else {

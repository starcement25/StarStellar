...
class APNotification extends APNSBase{
  ...
  /**
   * Title to be shown on receival of Push Notification
   * 
   *
   * @var string
   */
  protected $_title;
   ...
   /**
   * Image to be shown on receival of Push Notification
   * 
   *
   * @var string
   */
  ...
  /**
   * Sets Push Notification image_url
   *
   * @param string $title
   */
  ...
  private function __getNotificationPayload()
  {
    ...
    $notificationBody['aps']['alert'] = array('title' =>"","body"=>$this->_message);
	$notificationBody['aps']['mutable-content'] = 1;
	$notificationBody['aps']['content-available'] = 1;
	# Add title if set
    if($this->_title !== NULL && is_string($this->_title))
      $notificationBody['aps']['alert']['title'] = $this->_title;
	  # Add title if set
    if($this->_imageUrl !== NULL && is_string($this->_imageUrl))
      $notificationBody['mediaUrl'] = $this->_imageUrl;
...
$result = "";
    ...
    $result = fwrite($fp, $this->__getNotification());

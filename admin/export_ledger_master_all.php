...
$date_of_correction_in_point_calculation_as_per_new_SOP = "2020-03-19 00:00:00";
$te_master = "te_master";
...
if($whr_str!=""){
	// $new_whr_str = " where $ledger_master.`ldgr_datetime` >= '$date_of_correction_in_point_calculation_as_per_new_SOP' AND ".$whr_str;
		$new_whr_str = " where 
		$ledger_master.`user_id` != ''
		AND ".$whr_str;
		/*
		 
		AND $ledger_master.`related_id` != ''
		$ledger_master.`ldgr_datetime` >= '$date_of_correction_in_point_calculation_as_per_new_SOP'
		AND */
	}else{
...
$qry = "select
$ledger_master.*,
$engineer_master.`e_name`,
$engineer_master.`e_mobile`,
$engineer_master.`te_code`,
$te_master.`te_name`,
$te_master.`branch_code`,
$te_master.`te_mobile_no`
from $ledger_master
left join $engineer_master
on $ledger_master.`user_id`=$engineer_master.`eid`
left join $te_master
on $engineer_master.`te_code`=$te_master.`te_code`
$new_whr_str
order by
$ledger_master.`user_id` asc,
$ledger_master.`ldgr_datetime` asc,
$ledger_master.`ldgr_id` asc";
...
$output .= '"TE CODE","Order No","OLD Order No","Gift ID","Description","Gift Point","Engineer Name","Engineer Phn No","Branch","TE Name","TE Mobile","Shipping Address","Shipping Pin","Point Added","Point Reduced","TDS","Available Points","Creation Date"';
...
$r_description = $row1["description"] ? clean_csv_value($row1["description"]) : "";
...
// ======================================================
// Engineer Wise Running Available Balance Calculation
// Available Points = Total Point Added - Total Point Reduced
// ======================================================



static $engineer_balance = array();

if (!isset($engineer_balance[$r_engineer_id])) {
    $engineer_balance[$r_engineer_id] = 0;
}

$point_added   = (float)$r_point_earned;
$point_reduced = (float)$r_point_redeem;

// Running balance update
$engineer_balance[$r_engineer_id] =
    $engineer_balance[$r_engineer_id]
    + $point_added
    - $point_reduced;

// Final available balance
$remaining_balance = $engineer_balance[$r_engineer_id];


$the_branch_code_selected = $row1["branch_code"];
$related_id = $row1["related_id"] ? trim($row1["related_id"]) : "";

//new logic implemented on 13-04-2026
 $gift_order_master_id = $row1["gift_order_master_id"] ? trim($row1["gift_order_master_id"]) : "";
 $gift_order_master_id_old = $row1["gift_order_master_id"] ? trim($row1["gift_order_master_id"]) : "";
if($gift_order_master_id !=''){
	$sql_ord="SELECT `order_id`, `g_order_id` FROM `gift_order_master` where `g_order_id`='".$gift_order_master_id."'";
	$sql_ord_id = mysqli_query($conn,$sql_ord);
	$ord_rowgift_id=mysqli_fetch_assoc($sql_ord_id);
	$gift_order_master_id = $ord_rowgift_id["order_id"];
	$gift_order_new_ord_id = $ord_rowgift_id["g_order_id"];
}
			...
			$current_date = date('Y-m-d');
				...
				$old_order_id='';
				 if (strtotime($ldgr_datetime) <= strtotime($acknowledgement_rule_start_date)) {
					$old_order_id=($gift_order_master_id_old!=''?$gift_order_master_id_old:$the_g_order_id);
				 }else{
					$g_order_id=($gift_order_master_id!=''?$gift_order_master_id:$g_order_id_new);

				 }

				 if($gift_order_master_id !=''){
					$old_order_id=$gift_order_new_ord_id;
					$g_order_id=$gift_order_master_id;
				 }
...
if($r_point_earned!="" ){
			...
			if($rowgift_id["remarks"]!=''){
			$r_description = $rowgift_id["remarks"];
			}
...
//sk add end line start 10-11-12

$r_te_code                = clean_csv_value($r_te_code);
$g_order_id               = clean_csv_value($g_order_id);
$old_order_id             = clean_csv_value($old_order_id);
$gift_id                  = clean_csv_value($gift_id);
$product_point            = clean_csv_value($product_point);
$r_description            = clean_csv_value($r_description);
$r_engineer_name          = clean_csv_value($r_engineer_name);
$r_engineer_mobile        = clean_csv_value($r_engineer_mobile);
$the_branch_code_selected = clean_csv_value($the_branch_code_selected);
$r_te_name                = clean_csv_value($r_te_name);
$r_te_mobile_no           = clean_csv_value($r_te_mobile_no);
$address                  = clean_csv_value($address);
$pin                      = clean_csv_value($pin);
$r_point_earned           = clean_csv_value($r_point_earned);
$r_point_redeem           = clean_csv_value($r_point_redeem);
$tds                      = clean_csv_value($tds);
$remaining_balance        = clean_csv_value($remaining_balance);
$ldgr_datetime            = clean_csv_value($ldgr_datetime);

$output .= '"'.$r_te_code.'","'.$g_order_id.'","'.$old_order_id.'","'.$gift_id.'","'.$r_description.'","'.$product_point.'","'.$r_engineer_name.'","'.$r_engineer_mobile.'","'.$the_branch_code_selected.'","'.$r_te_name.'","'.$r_te_mobile_no.'","'.$address.'","'.$pin.'","'.$r_point_earned.'","'.$r_point_redeem.'","'.$tds.'","'.$remaining_balance.'","'.$ldgr_datetime.'"';
$output .="\n";
}

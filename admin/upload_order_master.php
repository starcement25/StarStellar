...
// ===================Pending Reason===================
$pending_reason= [];
$reasfltr = "select `id`,`reason` from `order_pending_reason` order by `id` asc";
$reasonfltr = mysqli_query($conn,$reasfltr);
while($row1reasonfltr=mysqli_fetch_assoc($reasonfltr)){
	$pend_reason = $row1reasonfltr["reason"];
    $pend_id = $row1reasonfltr["id"];
    $pending_reason[$pend_id]=$pend_reason;
    
}
//echo '<pre>';print_r($pending_reason);die;
/* ================= STATUS RULES ================= */
$statusRules = array(
    'PENDING' => array(
...
while (($data = fgetcsv($handle)) !== false) {
    ...
    if ($remarks != '') {
        if($new_status == 'PENDING' && isset($pending_reason[$remarks]) & $pending_reason[$remarks]!=''){
            $update[] = "pending_reason='".mysqli_real_escape_string($conn,$pending_reason[$remarks])."'";
        }else{

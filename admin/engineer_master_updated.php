...
$page_name = "engineer_master_updated.php";
...
if(isset($_GET["dclr_engid"]) && @$_GET["dclr_engid"]!=""){
	$dclr_engid = $_GET["dclr_engid"] ? trim($_GET["dclr_engid"]) : "";
	if($dclr_engid!=""){
		$sql_logout = "UPDATE $engineer_master SET `registration_id`='', `device_type`='', `device_id`='', `app_version`='' WHERE `eid`='$dclr_engid'";
		$res_logout = mysqli_query($conn, $sql_logout);
	}
	$msg_txt = "Engineer logged out from other device successfully.";
	header("location:".$page_name."?msg_txt=".$msg_txt);
}
                ...
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="card">
                        <div class="header">
                          ...
                          <a href="export_engineer_point_clarification_master.php" class="btn bg-red waves-effe">Export Point</a>&nbsp;&nbsp;
...
<a href="javascript:void(0);" class="btn bg-red waves-effe delete_engineer" dlt_engid="<?php echo $the_eid;?>">Delete</a>
	<?php }?>
	<?php if(strtoupper($the_access_user_type)=="ADMIN" && $device_type != ""){?>
<a href="javascript:void(0);" class="btn bg-red waves-effe logout_engineer" dclr_engid="<?php echo $the_eid;?>">Device Clearance</a>
	<?php }?>
</span>
</div>
</td>
...
jQuery(".logout_engineer").click(function(){
	var dclr_engid = jQuery(this).attr("dclr_engid");
	if(dclr_engid!=""){
		var cb = confirm("Do you want to log out the currently logged-in device?");
		if(cb == true){
			window.location = "<?php echo $page_name;?>?dclr_engid="+dclr_engid;
		}
	}
});	
...
jQuery('.sl_eng_sts').change(function(){
		...
		if(eng_sts!='' && eng_id!=''){
			var ldr_elmnt = jQuery("#sl_eng_sts_ldr_"+eng_id);
				ldr_elmnt.html(imgs);
				jQuery.ajax({
				url: 'ajax_update_eng_status_updated.php',
				type: 'post',
				dataType: 'json',
